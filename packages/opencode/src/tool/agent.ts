...
async function copyMessageHistory(sourceSessionID: string, targetSessionID: string, stripLastUserMessage: boolean = true) {
  ...
  if (stripLastUserMessage) {
    // Find the last user message (which triggered this AgentTool)
...
export const AgentTool = Tool.define({
  ...
  parameters: z.object({
    ...
    sessionID: z.string().optional().describe("Resume conversation with existing agent session ID. If not provided, creates new agent session. Use this to continue conversations with previously created agents."),
  }),
  async execute(params, ctx) {
    ...
    if (params.sessionID) {
      try {
        ...
        throw new Error(`Failed to resume agent session: session ${params.sessionID} not found`)
      }
    } else {
    ...
    unsub()
    const agentResult = result.parts.findLast((x) => x.type === "text")!.text
    const embeddedResult = `<result>${agentResult}</result>
<metadata>
sessionID: ${session.id}
title: ${params.description}
modelID: ${modelID}
